...
import { 
    ...
    Search, Calendar, ChevronDown, PhilippinePeso
} from "lucide-react"
                    ...
                    {loading ? (
                        ...
                        <div key={p._id || p.id} className="bg-white dark:bg-zinc-900 p-6 rounded-[2rem] border border-slate-100 dark:border-zinc-800 flex items-center gap-6 hover:shadow-2xl hover:shadow-slate-200/50 dark:hover:shadow-black/50 transition-all group">
                            <div className={cn("w-14 h-14 rounded-2xl flex items-center justify-center border", 
                                ...
                                {p.type === "Tattoo" ? <Paintbrush size={22} /> : <Scissors size={22} />}
                            </div>
                            <div className="flex-1 min-w-0">
                                <h3 className="text-sm font-black uppercase tracking-tight truncate">{p.name}</h3>
                                <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">{p.artist} • {p.type}</p>
                            </div>
                        ...
                        <div className="p-10 max-h-[90vh] overflow-y-auto">
                            ...
                            <form onSubmit={handleSave} className="space-y-6">
                                ...
                                <div className="space-y-5">
                                    <div>
                                        <label className="text-[10px] font-black uppercase text-slate-400 dark:text-zinc-500 ml-1 tracking-widest">Promo Name</label>
                                        <input 
                                            required className="w-full bg-slate-50 dark:bg-zinc-900 p-4 rounded-2xl text-[13px] font-bold border-2 border-transparent focus:border-black dark:focus:border-white focus:bg-white dark:focus:bg-zinc-800 outline-none transition-all dark:text-white"
                                            value={editingPromo.name} onChange={e => setEditingPromo({...editingPromo, name: e.target.value})}
                                        />
                                    </div>
