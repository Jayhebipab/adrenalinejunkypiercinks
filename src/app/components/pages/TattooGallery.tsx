...
interface GalleryRow {
    id: number;
    placement: string;
    files: File[]; // Pinalitan ang string[] ng File[] para sa Cloudinary
    previews: string[]; // Para sa preview bago i-upload
    artistId: string;
}
...
export default function TattooGallery() {
    ...
    const uploadToCloudinary = async (file: File) => {
        ...
        formData.append("upload_preset", "adrenalinejunkypiercinks"); // Palitan kung may separate preset ka para sa tattoo
                            ...
                            {rows.map((row, index) => (
                                <div key={row.id} className="p-6 bg-zinc-50 rounded-3xl border border-zinc-100 space-y-4 relative">
                                    ...
                                    <label className="flex flex-col items-center justify-center py-8 border-2 border-dashed border-zinc-200 hover:border-black rounded-2xl cursor-pointer transition bg-white group">
                                        ...
                                        <span className="text-[9px] font-black uppercase tracking-widest text-zinc-400">Upload Tattoo Images</span>
