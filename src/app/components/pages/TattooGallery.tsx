...
export default function TattooGallery() {
    ...
    const removeRow = (id: number) => {
        ...
        rowToRemove?.previews.forEach(p => URL.revokeObjectURL(p));
    ...
    const saveAll = async () => {
        ...
        setUploading(true);
        try {
            // FIXED: Added TattooEntry[] type to fix TS7034 & TS7005
            const allTattoos: TattooEntry[] = []; 
            
            for (const row of rows) {
                const artist = artists.find(a => a._id === row.artistId);
                const uploadPromises = row.files.map(file => uploadToCloudinary(file));
            ...
            if (res.ok) {
                toast.success("Tattoos added to portfolio!");
    ...
    return (
        <div className="min-h-screen p-4 md:p-8 text-slate-900 ">
            ...
            <div className="max-w-6xl mx-auto space-y-10">
                <div className="flex justify-between items-center bg-zinc-900 p-8 rounded-[2.5rem] text-white shadow-2xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="relative group">
                        <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-black transition-colors" />
                        <input 
                            type="text" 
                            placeholder="SEARCH BY PLACEMENT..." 
                            className="w-full pl-16 pr-8 py-5 bg-white border border-slate-100 rounded-3xl shadow-sm outline-none font-bold focus:ring-2 ring-black transition-all text-xs tracking-widest uppercase"
                            value={search}
                    ...
                    {fetching ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {filteredItems.map((item) => (
                                <div 
                                    key={item._id} 
                                    // FIXED: Rounded from [2rem] to 4xl
                                    className="bg-white p-4 rounded-4xl border border-slate-100 flex items-center gap-5 group hover:shadow-2xl transition-all relative overflow-hidden"
                                >
                                    ...
                                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all">
                                        <button onClick={() => setSelectedImg(item.image)} className="p-2.5 bg-slate-50 rounded-xl hover:bg-black hover:text-white transition-colors">
                                            <Maximize2 size={16} />
                                        </button>
                                        <button onClick={() => { setEditingItem(item); setIsEditOpen(true); }} className="p-2.5 bg-slate-50 rounded-xl hover:bg-black hover:text-white transition-colors">
                                            <Edit3 size={16} />
                                        </button>
                                        <button onClick={() => deleteItem(item._id)} className="p-2.5 bg-red-50 text-red-500 rounded-xl hover:bg-red-500 hover:text-white transition-colors">
                                            <Trash2 size={16} />
                                        </button>
                                    </div>
                                </div>
            ...
            {isUploadOpen && (
                <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-md flex items-center justify-center p-4">
                    <div className="bg-white w-full max-w-2xl rounded-[3rem] p-8 md:p-12 shadow-2xl animate-in zoom-in duration-300 relative max-h-[90vh] flex flex-col border">
                        <button onClick={() => setIsUploadOpen(false)} className="absolute top-10 right-10 p-2 hover:bg-slate-100 rounded-full transition-colors">
                            <X size={24} />
                        </button>
                        
                        <div className="mb-8">
                            <h2 className="text-4xl font-black italic uppercase tracking-tighter">New Tattoo</h2>
                            <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest">Batch Upload Entries</p>
                        </div>

                        <div className="space-y-6 overflow-y-auto pr-2 flex-1">
                            {rows.map((row, index) => (
                                <div key={row.id} className="p-6 bg-slate-50 rounded-3xl border border-slate-100 space-y-4 relative">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div className="relative">
                                            ...
                                            <select
                                                className="w-full pl-12 pr-4 py-4 bg-white border rounded-2xl outline-none focus:ring-2 ring-black font-bold text-xs uppercase appearance-none"
                                                value={row.artistId}
                                        ...
                                        <div className="relative">
                                            ...
                                            <input
                                                ...
                                                className="w-full pl-12 pr-4 py-4 bg-white border rounded-2xl outline-none focus:ring-2 ring-black font-bold text-xs uppercase"
                                                value={row.placement}
                                    ...
                                    <label className="flex flex-col items-center justify-center py-10 border-2 border-dashed border-slate-200 hover:border-black rounded-3xl cursor-pointer transition bg-white group">
                                        <UploadCloud className="w-8 h-8 text-slate-300 group-hover:text-black mb-2" />
                                    ...
                                    {row.previews.length > 0 && (
                                        <div className="flex flex-wrap gap-2 pt-2">
                                            {row.previews.map((src, i) => (
                                                <img key={i} src={src} className="w-14 h-14 rounded-xl object-cover border" />
                                            ))}
                                        </div>
                                    )}
                                </div>
                        ...
                        <div className="flex gap-4 mt-8 pt-6 border-t">
                            <button onClick={addRow} className="flex-1 py-5 border border-slate-200 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-slate-50 transition-colors">Add Row</button>
                            <button 
                                onClick={saveAll} 
                                disabled={uploading} 
                                // FIXED: Class from flex-[2] to flex-2
                                className="flex-2 py-5 bg-black text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-2xl disabled:opacity-50"
                            >
            ...
            {isEditOpen && editingItem && (
                <div className="fixed inset-0 z-60 bg-black/95 backdrop-blur-xl flex items-center justify-center p-4">
                    <div className="bg-white w-full max-w-lg rounded-[2.5rem] p-8 md:p-10 shadow-2xl animate-in zoom-in duration-200 relative border">
                        <button onClick={() => setIsEditOpen(false)} className="absolute top-8 right-8 p-2 hover:bg-slate-100 rounded-full">
                            <X size={20} />
                        </button>
                        <div className="mb-6">
                        ...
                        <img src={editingItem.image} className="w-full h-56 object-cover rounded-3xl border mb-6" />
                        <div className="space-y-4">
                            <div className="space-y-2">
                                <label className="text-[9px] font-black uppercase tracking-widest text-slate-400 ml-2">Assigned Artist</label>
                                <select
                                    className="w-full px-6 py-4 bg-slate-50 border rounded-2xl outline-none focus:ring-2 ring-black font-bold text-xs uppercase"
                        ...
                        <button 
                            ...
                            className="mt-8 w-full py-5 bg-black text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl disabled:opacity-50"
                        >
            ...
            {selectedImg && (
                ...
                >
                    <img 
                        ...
                        className="max-w-full max-h-[90vh] rounded-4xl object-contain shadow-2xl animate-in zoom-in duration-300" 
                    />
                </div>
            )}
        </div>
