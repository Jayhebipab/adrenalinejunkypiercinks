...
import {
    ...
    AlertCircle, Users, ImageIcon, Syringe, MapPin
} from "lucide-react"
...
function validateRow(data: { placement: string; artistId: string }): FormErrors {
    ...
    if (!data.artistId) errors.artistId = "Please select a Master Piercer.";
    return errors;
}
...
export default function PiercingGallery() {
    ...
    const handleImageChange = (rowId: number, files: FileList | null) => {
        ...
        if (allFiles.some(f => !ALLOWED_TYPES.includes(f.type))) {
                            ...
                            {rows.map((row, index) => (
                                <div key={row.id} className="p-6 bg-muted/40 rounded-2xl border border-border space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        {/* Master Piercer Select */}
                                        <FormField label="Master Piercer" error={rowErrors[row.id]?.artistId} input={
                                            <div className="relative">
                    ...
                    <div className="bg-card border border-border w-full max-w-sm rounded-[2.5rem] shadow-2xl animate-in fade-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
                        <div className="p-8 space-y-6">
                            ...
                            <div className="space-y-4">
                                <FormField label="Master Piercer" error={editErrors.artistId} input={
                                    <select
