...
export default function TattooGallery() {
  ...
  const updateStatus = async (id: string, newStatus: string) => {
    ...
    const targetBooking = items.find(b => b.id === id);
    ...
    try {
      ...
      if (res.ok) {
        ...
        // ✅ AUDIT LOG
        await logAudit({
          action: 'FINISHED PROJECT',
          details: `Closed project of ${targetBooking?.name ?? id} (${targetBooking?.service ?? '—'}) — Artist: ${selectedArtist}, Final Price: ₱${Number(price).toLocaleString()}, Materials: ${cleanedProducts.map(p => `${p.name} x${p.quantity}`).join(', ')}`,
        });
    ...
    try {
      ...
      if (res.ok) {
        ...
        await logAudit({
          ...
          details: `Moved project of ${targetBooking?.name ?? id} (${targetBooking?.service ?? '—'}) back to Pending status`,
        });
                                ...
                                <div className="space-y-6 py-4">
                                  ...
                                  <div className="space-y-3">
                                    <div className="flex justify-between items-center">
                                      ...
                                      <button onClick={addProductRow} className="text-primary hover:scale-110 transition-transform">
                                        <PlusCircle size={18} />
                                      </button>
                                    </div>
