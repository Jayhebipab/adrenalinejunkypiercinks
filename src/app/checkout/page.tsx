...
interface FormErrors { phone?: string; address?: string; }

function validateForm(data: { phone: string; address: string }): FormErrors {
  const errors: FormErrors = {};
  const cleanPhone = data.phone.replace(/\s/g, "");
  if (!cleanPhone)                          errors.phone   = "Contact number is required.";
  else if (!PH_PHONE_REGEX.test(cleanPhone)) errors.phone   = "Must be a valid PH number (09XXXXXXXXX or +639XXXXXXXXX).";
  if (!data.address.trim())                 errors.address = "Complete address is required.";
  else if (SPECIAL_CHAR_REGEX.test(data.address)) errors.address = "Address contains invalid special characters.";
  return errors;
}
...
// ─── ICON HELPER ──────────────────────────────────────────────────────────────
function PaymentIcon({ icon, className = "size-5" }: { icon: string; className?: string }) {
  const icons: Record<string, React.ReactNode> = {
    gcash:  <Smartphone className={className} />,
    bank:   <Building2  className={className} />,
...
export default function CheckoutPage() {
  ...
  // ─── LOAD ITEMS, PAYMENT METHODS, VAT ────────────────────────────────────
  useEffect(() => {
    // Payment methods
    ...
    if (currentItems.length > 0) {
      ...
      const calc = currentItems.reduce((acc: number, item: any) =>
        acc + (Number(item.cost_price || item.selling_price || 0) * item.quantity), 0);
      setSubtotal(calc);
    } else {
      router.push('/shop');
    }
  ...
  const handlePlaceOrder = () => {
    ...
    if (!selectedMethodId) { alert("Please select a payment method."); return; }
    setFormErrors({});
    setLoading(true);
    setTimeout(() => { setLoading(false); setShowQRModal(true); }, 800);
  };
  ...
  const handleVerifyAndSubmit = async () => {
    if (!screenshot || !selectedMethod) return;
    setIsUploading(true);
    ...
    try {
      ...
      if (res.ok) {
        ...
        else { localStorage.removeItem('adrenaline_cart'); window.dispatchEvent(new Event('cart-updated')); }
      } else {
        setIsUploading(false);
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <section className="bg-zinc-900/50 border border-white/5 p-8 rounded-[2.5rem]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <input type="text" placeholder="FULL NAME" value={shippingData.name} readOnly
                ...
                <input type="email" placeholder="EMAIL" value={shippingData.email} readOnly
                  className="bg-zinc-800/30 border border-white/5 rounded-xl p-4 text-[10px] font-bold tracking-widest uppercase text-zinc-500 cursor-not-allowed" />

                <div>
                  <input
                    type="text" placeholder="09XXXXXXXXX" value={shippingData.phone}
                    onChange={e => {
                    ...
                    className={cn(
                      ...
                      formErrors.phone ? "border-red-500/60 bg-red-500/5 focus:border-red-500" : "border-white/10 focus:border-orange-500"
                    )}
                  />
                ...
                <div className="md:col-span-2">
                  <textarea
                    ...
                    rows={3} value={shippingData.address}
                    onChange={e => {
                    ...
                    className={cn(
                      ...
                      formErrors.address ? "border-red-500/60 bg-red-500/5 focus:border-red-500" : "border-white/10 focus:border-orange-500"
                    )}
                  />
            ...
            <section className="bg-zinc-900/50 border border-white/5 p-8 rounded-[2.5rem]">
              ...
              {paymentMethods.length === 0 ? (
                <div className="text-center py-8 text-zinc-600">
                  <QrCode className="mx-auto mb-3 opacity-30" size={32} />
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        selectedMethodId === method.id
                          ...
                          : "border-white/5 bg-zinc-800/50 opacity-50 hover:opacity-80"
                      )}
                    >
                      <PaymentIcon icon={method.icon} className="size-6" />
                      <span className="text-[9px] font-black tracking-widest uppercase text-center leading-tight">
                        {method.type}
                      </span>
                      {method.number && (
                        <span className="text-[8px] font-bold text-zinc-500 font-mono truncate w-full text-center">
                          {method.number}
                        </span>
                      )}
                    </button>
          ...
          >
            ...
            >
              ...
              <div className="flex items-center justify-center gap-2 mb-2">
                <PaymentIcon icon={selectedMethod.icon} className="size-5 text-orange-500" />
                <h3 className="text-white font-black uppercase text-2xl italic">
                  {selectedMethod.type}
                </h3>
              </div>
              ...
              {/* QR Code */}
              <div className="bg-white p-4 rounded-3xl mb-6 mx-auto shadow-lg w-full max-w-[280px] sm:max-w-[240px] md:max-w-[220px]">
                {selectedMethod.qr ? (
                  <img src={selectedMethod.qr} className="w-full aspect-square object-contain" alt="QR Code" />
                ) : (
              ...
              <div className="bg-white/5 border border-white/5 rounded-2xl p-4 mb-6 text-left space-y-3">
                <div>
                  ...
                  <p className="text-[11px] text-white font-black uppercase">{selectedMethod.name || "—"}</p>
                </div>
                {selectedMethod.number && (
                  <div>
                    <p className="text-[7px] text-zinc-500 font-black uppercase tracking-tighter mb-0.5">Account Number</p>
                    <p className="text-sm text-orange-500 font-mono font-black tracking-wider">{selectedMethod.number}</p>
                  </div>
                )}
              </div>
              ...
              <div className="border-t border-white/5 pt-6">
                {!screenshot ? (
                  <label className={cn(
                    ...
                    receiptError ? "border-red-500/40 bg-red-500/5 hover:bg-red-500/10" : "border-white/10 hover:bg-white/5"
                  )}>
                    ...
                    <input type="file" className="hidden" accept=".jpg,.jpeg,.png,image/jpeg,image/png" onChange={handleReceiptChange} />
                  </label>
                ) : (
                  <div className="relative w-full mb-4 group">
                    <img src={screenshot} className="w-full h-36 object-cover rounded-2xl border border-white/10" alt="Receipt" />
                    <button
                ...
                >
                  {isUploading
                    ? <span className="flex items-center justify-center gap-2"><Loader2 size={14} className="animate-spin" /> Uploading Receipt...</span>
                    : "Confirm & Submit"
                  }
                </button>
