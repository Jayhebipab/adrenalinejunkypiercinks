...
export default function ProductDetails() {
  ...
  const handleAddToCart = () => {
    ...
    const existingIndex = currentCart.findIndex((item: any) => item.id === (product.id || product._id));
  ...
  const handleDirectBuy = () => {
    ...
    // I-save natin itong specific item lang sa temporary checkout storage
    const checkoutItem = {
      id: product.id || product._id,
      name: product.name,
      cost_price: product.cost_price,
    ...
    router.push('/checkout'); // Lipat agad sa checkout page
  };
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-200">
      ...
      <main className="container mx-auto max-w-6xl px-6 py-32">
        ...
        <div className="flex flex-col lg:flex-row gap-12 lg:items-start justify-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full lg:max-w-md">
            <div className="relative aspect-[4/5] rounded-[2rem] overflow-hidden border border-white/5 bg-zinc-900 shadow-2xl">
              <img src={product.image || "/placeholder-gear.jpg"} className="w-full h-full object-cover" alt={product.name} />
              <div className="absolute inset-0 bg-gradient-to-t from-zinc-950/20 to-transparent" />
            </div>
          </motion.div>

          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="flex-1 space-y-8 py-4 text-left">
            <div className="space-y-4">
              ...
              <p className="text-3xl font-black text-orange-500 tracking-tighter">₱{(Number(product.cost_price) || 0).toLocaleString()}</p>
            </div>

            <div className="h-[1px] w-full bg-white/5" />

            <div className="space-y-3">
              <h3 className="text-[10px] font-black uppercase tracking-widest text-zinc-400 text-left">The Details</h3>
              <p className="text-zinc-500 leading-relaxed uppercase text-[11px] tracking-wide font-medium max-w-sm text-left">
                {product.description || "Premium studio essential crafted for precision and quality."}
              </p>
            </div>

            <div className="flex flex-wrap gap-3 py-2">
              <div className="flex items-center gap-2 text-[9px] font-black uppercase tracking-widest text-zinc-300 bg-white/5 px-4 py-3 rounded-xl border border-white/5 backdrop-blur-sm">
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-4">
              <button 
                ...
                className="flex-1 bg-orange-600 hover:bg-orange-700 text-white py-5 rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] transition-all active:scale-95 shadow-lg shadow-orange-600/20"
              >
                Direct Buy
              </button>
              <button 
              ...
              .map((item: any) => (
                ...
                >
                  <div className="aspect-[4/5] rounded-2xl overflow-hidden bg-zinc-900 border border-white/5 mb-3">
                    <img src={item.image || "/placeholder-gear.jpg"} className="w-full h-full object-cover opacity-60 group-hover:opacity-100 transition-all duration-500 group-hover:scale-105" alt={item.name} />
                  </div>
                  <h4 className="text-white font-black uppercase text-[9px] tracking-tight truncate px-1">{item.name}</h4>
                  <p className="text-orange-500 font-black text-[9px] px-1">₱{(Number(item.cost_price) || 0).toLocaleString()}</p>
                </motion.div>
            ))}
          </div>
        </div>
